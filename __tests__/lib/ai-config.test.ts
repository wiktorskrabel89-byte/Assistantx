...
describe("per-model behavior profiles", () => {
  ...
  it("uses explicit GPT OSS coding temperature and reasoning prompt profile", () => {
    expect(getModelTemperature("openai/gpt-oss-120b:free", { isCodeRequest: true })).toBe(0.2);
    expect(getModelPromptText("openai/gpt-oss-120b:free", true)).toMatch(/advanced reasoning ai/i);
  });
});
