...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { status, payload } = buildLinkedAccountsError(error, 'Failed to process GitHub linked account request.');
...
export async function DELETE(_req: NextRequest) {
  ...
  try {
    ...
    const { status, payload } = buildLinkedAccountsError(error, 'Failed to unlink GitHub account.');
    return NextResponse.json(payload, { status });
  }
