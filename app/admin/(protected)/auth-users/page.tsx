...
export default async function AdminAuthUsers({
  ...
  const { users, total, nextPage } = await listAuthUsers(page, PAGE_SIZE);
    ...
    <div>
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm text-white/40">
          Actual Supabase <code className="text-white/70">auth.users</code>. Deletion is permanent and cascades according to your DB schema.
              ...
              {users.map((u) => (
                <tr key={u.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3 text-white/80 font-mono text-xs">
                    {reveal ? u.email || "—" : maskEmail(u.email)}
                  </td>
      ...
      <div className="mt-4 flex items-center justify-between text-xs text-white/50">
        ...
        <div className="flex gap-2">
          ...
          {nextPage && (
            <Link
              href={{ pathname: "/admin/auth-users", query: buildQuery({ page }) }}
              className="rounded-full border border-white/10 px-3 py-1 hover:border-white/20"
            >
              Next →
            </Link>
